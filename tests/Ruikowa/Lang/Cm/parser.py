...
namespace     = globals()
recurSearcher = set()
Any = LiteralParser('^((?!/\*|\*/|\n)[\s\S])*$', name = 'Any', isRegex = False)
multilineComment = AstParser([LiteralParser('/*', name='\'/*\''),SeqParser([DependentAstParser([Ref('Any')],[Ref('NEWLINE')],[Ref('multilineComment')])]),LiteralParser('*/', name='\'*/\'')], name = 'multilineComment')
...
Constant = LiteralParser('null|false|true', name = 'Constant', isRegex = False)
...
statement = AstParser([DependentAstParser([Ref('flowControl')],[Ref('declaration')],[Ref('flowControlSign'),SeqParser([Ref('Identifier')], atmost = 1)],[Ref('expression')]),SeqParser([Ref('EOL')], atmost = 1)], name = 'statement')
flowControlSign = LiteralParser('break|return|continue', name = 'flowControlSign', isRegex = False)
...
structDeclaration = AstParser([LiteralParser('struct', name='\'struct\''),Ref('Identifier'),LiteralParser('{', name='\'{\''),SeqParser([SeqParser([Ref('I')]),Ref('variableDeclarationEntry'),SeqParser([LiteralParser(',', name='\',\'')], atmost = 1),SeqParser([Ref('I')])]),LiteralParser('}', name='\'}\'')], name = 'structDeclaration', toIgnore = [{I},{}])
...
variableDeclarationEntryList = AstParser([SeqParser([Ref('I')]),Ref('variableDeclarationEntry'),SeqParser([SeqParser([Ref('I')]),LiteralParser(',', name='\',\''),SeqParser([Ref('I')]),Ref('variableDeclarationEntry')]),SeqParser([Ref('I')])], name = 'variableDeclarationEntryList', toIgnore = [{I},{}])
variableDeclaration = AstParser([DependentAstParser([LiteralParser('let', name='\'let\'')],[LiteralParser('var', name='\'var\'')]),Ref('variableDeclarationEntry'),SeqParser([LiteralParser('=', name='\'=\''),SeqParser([Ref('I')]),Ref('expression')], atmost = 1)], name = 'variableDeclaration', toIgnore = [{I},{}])
...
expressionList = AstParser([SeqParser([Ref('I')]),Ref('expression'),SeqParser([SeqParser([Ref('I')]),LiteralParser(',', name='\',\''),SeqParser([Ref('I')]),Ref('expression')]),SeqParser([Ref('I')])], name = 'expressionList', toIgnore = [{I},{}])
Trailer = AstParser([Ref('Call')],[Ref('Access')], name = 'Trailer')
Call = AstParser([LiteralParser('(', name='\'(\''),SeqParser([Ref('expressionList')], atmost = 1),LiteralParser(')', name='\')\''),SeqParser([Ref('LambdaDef')], atmost = 1)], name = 'Call', toIgnore = [{I},{}])
Access = AstParser([SeqParser([Ref('I')]),LiteralParser('.', name='\'.\''),Ref('Identifier')], name = 'Access', toIgnore = [{I},{}])
...
statements = AstParser([SeqParser([SeqParser([Ref('I')]),SeqParser([Ref('statement')]),SeqParser([Ref('I')])])], name = 'statements', toIgnore = [{I},{}])
